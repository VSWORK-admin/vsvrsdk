...
public class ETCAxis {
	...
	public void DoDirectAction(){
		...
		if (directTransform){
			...
			Vector3 directaxis = localAxis * axisSpeedValue;
			int ret = VSWorkSDK.AvatarActionController.Instance.PreCheckAction(directAction);
			if (ret == -1)
				return;
			...
			VSWorkSDK.AvatarActionController.Instance.DoDirectAction(directAction, directaxis);
		}
